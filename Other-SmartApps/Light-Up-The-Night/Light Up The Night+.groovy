...
definition(
    ...
    description: "Turn specific switches on when it gets dark and changes the mode (optionally). Will also turn off specific switches and optionally change modes when it becomes light again based on brightness from a connected light sensor.",
...
preferences {
    ...
    section("Turn off lights/set mode when brightness above specific luminosity") {
		input "lightsOff", "capability.switch", multiple: true, title: "Lights/Switches", required: true
		input "luxOff", "number", title: "Lux Setting", required: true, description:50
...
def illuminanceHandler(evt) {
	...
	else if (lastStatus != "off" && evt.integerValue > luxOff) {
		lightsOff.off()
